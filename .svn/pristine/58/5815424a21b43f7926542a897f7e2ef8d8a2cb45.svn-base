...
- (UIView *)creatHeadView{
    ...
    phoneL.text = [NSString replaceStringNull:Datadic[@"user"][@"nickname"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
   NSArray *titleArr = @[@"消息提醒",@"意见反馈",@"关于我们",@"系统设置",@"授权登录"];
//    NSArray *titleArr = @[@"消息提醒",@"意见反馈",@"系统设置",@"授权登录"];
    NSArray *imageArr = @[@"me_message",@"me_face",@"me_me",@"me_setting",@"me_shouquan"];
//    NSArray *imageArr = @[@"me_message",@"me_face",@"me_setting",@"me_shouquan"];
    ...
    if (indexPath.row == 0) {
        ...
        if (instance.user.unionid.length >0) {
            cell.typeL.text = @"已授权";
            cell.typeL.textColor = COLOR(77 , 76, 76);
        }else{
            cell.typeL.text = @"未授权";
            cell.typeL.textColor = [UIColor redColor];
        }
        cell.cleanL.backgroundColor = [UIColor whiteColor];
    }else{
        cell.typeL = nil;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
        if (indexPath.row == 0) {
            ...
            [[UIHelper sharedSingleton] pushVC:@"NewsWebViewController" vc:self parames:@{@"Titlestr":@"关于我们",@"str":@"https://api.cloudcpc.com/admin/show/about"}];
        }
    ...
    [[UMSocialManager defaultManager] getUserInfoWithPlatform:1  currentViewController:self completion:^(id result, NSError *error) {
        ...
        if (error) {
            ...
            if ([result isKindOfClass:[UMSocialUserInfoResponse class]]) {
                ...
                NSDictionary *biz = @{@"openid":resp.openid,@"unionId":resp.unionId,@"headimgurl":resp.iconurl,@"nickname":resp.name};
                [ConnectionRequestMgr POSTSessionWithUrl:WechatLogin parameter:biz successBlock:^(NSDictionary *dict) {
                    [weakSelf hideHUD];
                    if ([dict[@"code"] integerValue] == 1) {
