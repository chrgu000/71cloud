...
class Exampaper extends Base
{
    ...
    {
        $data = model('Exampaper')->where('branchs_id',session('branchadmin_info.branchs_id'))->select();
    ...
    public function create()
    {
        ...
        $data = model('ExamActivity')->where('branchs_id',$branchs_id)->select();
...
//        dump($request->param());
        ...
        $branchs_id = session('branchadmin_info.branchs_id');
        // 获取题目表中对应组织所有的试题的总数量
        $count = model('ExamQuestions')->where('branchs_id',$branchs_id)->count();
        // 判断如果用户选择的题目数量大于其所属组织的题目总数量就为不合法进行跳转
        if($question_num > $count)
        {
            $this->error('题目总数量选择不合法，请重新选择');
        }
        $all_questionid = \think\Db::table('c_exam_questions')->where('branchs_id',$branchs_id)->column('id');
