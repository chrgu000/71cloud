...
#import "StoreViewController.h"
@interface HomeViewController ()<UIScrollViewDelegate,SDCycleScrollViewDelegate,SGAdvertScrollViewDelegate>
{
...
- (void)creatUI{
    ...
    NSArray *iamgeVArr = @[@"home_news",@"home_school",@"home_Law",@"home_Integral",@"home_photograph",@"home_Onlineassessment",@"home_Appstore",@"home_Supervision",@"home_heart",@"home_signup"];
    NSArray *titleArr = @[@"党建要闻",@"微党校",@"反腐倡廉",@"积分考核",@"党建相册",@"在线考试",@"应用商店",@"群众监督",@"一颗红心",@"活动报名"];
    for (int i = 0; i<2; i++) {
        for (int j = 0; j<5; j++) {
    ...
    banner1.image = [UIImage imageNamed:@"appstore-bg"];
    banner1.userInteractionEnabled = YES;
    [backBannerOne addSubview:banner1];
    
    UILabel *StoreTopL= [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_W -200, 12, 164, 16)];
    StoreTopL.text = @"党建小程序应用商店";
    StoreTopL.textAlignment = 2;
    StoreTopL.textColor = COLOR(227, 43, 44);
    StoreTopL.font = FontBold(16);
    [banner1 addSubview:StoreTopL];
    
    UILabel *StoreDownL= [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_W -72-36, CGRectGetMaxY(StoreTopL.frame) + 7, 72, 20)];
    StoreDownL.text = @"更多应用";
    StoreDownL.textColor = COLOR(227, 43, 44);
    StoreDownL.layer.masksToBounds = YES;
    StoreDownL.layer.borderWidth = 0.5;
    StoreDownL.layer.borderColor = COLOR(232, 51, 41).CGColor;
    StoreDownL.layer.cornerRadius = 2;
    StoreDownL.textAlignment = 1;
    StoreDownL.font = Font(12);
    [banner1 addSubview:StoreDownL];
    
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(NotificationAction)];
    [banner1 addGestureRecognizer:tap];
...
//    }
    ...
    [banner2 sd_setImageWithURL:[NSURL URLWithString:AdArr[0][@"thumb"]] placeholderImage:[UIImage imageNamed:@"notice—bg"]];
    ...
    for (int i = 0; i<2; i++) {
        for (int j = 0; j<2; j++) {
            ...
            lookNum.text = [NSString stringWithFormat:@"%@",schoolArr[2*i+j][@"visit"]];
...
//    }
    ...
    [banner4 sd_setImageWithURL:[NSURL URLWithString:AdArr[1][@"thumb"]] placeholderImage:[UIImage imageNamed:@"banner-xinlihua"]];
    ...
    UIImageView *BackStar = [[UIImageView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(backBanner4.frame), SCREEN_W , 180)];
    ...
    AdvertScrollView = [[SGAdvertScrollView alloc] init];
    AdvertScrollView.backgroundColor = [UIColor clearColor];
    AdvertScrollView.frame = CGRectMake(24, CGRectGetMaxY(StarB.frame) + 20, 270, 60);
    AdvertScrollView.advertScrollViewStyle = SGAdvertScrollViewStyleMore;
    AdvertScrollView.bottomTitleColor = COLOR(227, 43, 44);
    AdvertScrollView.topTitleColor = COLOR(227, 43, 44);
    AdvertScrollView.titleFont = FontBold(16);
    NSMutableArray *tAry = [[NSMutableArray alloc] init];
    NSMutableArray *sAry = [[NSMutableArray alloc] init];
    int Count = 1;
        for (NSDictionary *dic in heartArr) {
            if (Count%2==0) {
                [sAry addObject:dic[@"title"]];
            }else{
                [tAry addObject:dic[@"title"]];
            }
            Count ++;
        }
    AdvertScrollView.topSignImages = @[@"home_starS", @"home_starS", @"home_starS",@"home_starS", @"home_starS", @"home_starS"];
    AdvertScrollView.bottomSignImages =  @[@"home_starS", @"home_starS", @"home_starS",@"home_starS", @"home_starS", @"home_starS"];
    AdvertScrollView.topTitles =[sAry copy];
    AdvertScrollView.bottomTitles = [tAry copy];
    AdvertScrollView.delegate =self;
  
    [BackStar addSubview:AdvertScrollView];
...
//            [[UIHelper sharedSingleton] pushVC:@"ExhibitionViewController" vc:self parames:nil];
            [[UIHelper sharedSingleton] pushVC:@"StoreViewController" vc:self parames:nil];
            break;
        case 7:
...
- (void)actionAD:(UITapGestureRecognizer *)sender{
    ...
    if ( url.length >0) {
        [[UIHelper sharedSingleton] pushVC:@"NewsWebViewController" vc:self parames:@{@"str":url,@"Titlestr":AdArr[sender.view.tag -20][@"name"]}];
    }
}
