...
- (void)GreateUI{
    ...
    [wenxinB setBackgroundImage:[UIImage imageNamed:@"wechat"] forState:UIControlStateNormal];
    ...
    [[UMSocialManager defaultManager] getUserInfoWithPlatform:1  currentViewController:self completion:^(id result, NSError *error) {
        ...
        if (error) {
            ...
            if ([result isKindOfClass:[UMSocialUserInfoResponse class]]) {
                ...
                NSLog(@"%@",message);
                WEAKSELF
                [weakSelf showHUD:nil];                NSDictionary *biz = @{@"uid":@"none",@"openid":resp.openid,@"unionId":resp.unionId,@"headimgurl":resp.iconurl,@"nickname":resp.name};                [ConnectionRequestMgr POSTSessionWithUrl:WechatLogin parameter:biz successBlock:^(NSDictionary *dict) {                    [weakSelf hideHUD];                    if ([dict[@"code"] integerValue] == 1) {                        if ([dict[@"type"] integerValue] == 3) {                            [[UIHelper sharedSingleton] pushVC:@"RegisterViewController" vc:self parames:@{@"type":@"1",@"wechatID":dict[@"data"][@"uid"]}];                        }else{                            ThePartySingTon *instance = [ThePartySingTon SharedInstance];                            instance.user = [UserLogin mj_objectWithKeyValues:dict[@"data"]];                            instance.isLogin = YES;                            AppDelegate *app = (AppDelegate *)[UIApplication sharedApplication].delegate;                            [app createRootViewController];                        }                                            }else{                        [weakSelf showError:dict[@"msg"]];                    }                } failBlock:^(NSString *errorStr) {                    [weakSelf hideHUD];                    [weakSelf showError:errorStr];                }];
            }else{
