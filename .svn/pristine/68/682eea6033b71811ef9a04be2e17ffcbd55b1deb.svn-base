...
class Contentad extends Base{
    ...
    public function index(){
        ...
        #获取已经选择的内容
        $company = explode(',', Db::name('company_resources')->where(['company_id' => $company_id])->value('ad'));
        $admin   = explode(',', Db::name('admin_resources')->where(['company_id' => $company_id])->value('ad'));
        ...
        return view('content/ContentAd/index');
    }
    ...
    public function edit(){
        ...
        if(Request::instance()->isGet()){
            ...
            //检测重名
            $adModel = db('CompanyAd');
            if($adModel -> where(array('name'=>$data['name'],'id'=>$id) ) -> find()){
                return json(['code' => 0,'msg' => '名称重复！']);
            }
