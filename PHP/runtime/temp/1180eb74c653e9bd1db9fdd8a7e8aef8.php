<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:82:"D:\PHPTutorial\WWW\71cloud\PHP\public/../application/admin\view\exambank\edit.html";i:1542088953;s:65:"D:\PHPTutorial\WWW\71cloud\PHP\application\admin\view\layout.html";i:1539913150;s:72:"D:\PHPTutorial\WWW\71cloud\PHP\application\admin\view\public\header.html";i:1541574066;s:72:"D:\PHPTutorial\WWW\71cloud\PHP\application\admin\view\public\footer.html";i:1541387770;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <title>七一云党建</title>
	...
	<form class="form form-horizontal" id="form-article-add" enctype="multipart/form-data">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $row['content']; ?>" placeholder="请输入题目" id="content" name="content">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>opt1：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $row['opt1']; ?>" placeholder="请输入选项答案" id="opt1" name="opt1">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>opt2：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $row['opt2']; ?>" placeholder="请输入选项答案" id="opt2" name="opt2">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>opt3：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $row['opt3']; ?>" placeholder="请输入选项答案" id="opt3" name="opt3">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>opt4：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $row['opt4']; ?>" placeholder="请输入选项答案" id="opt4" name="opt4">
			</div>
		</div>
			...
			<div class="formControls col-xs-8 col-sm-9">
				<span class="select-box" style="width:150px;">
					<select class="select" name="answer_k" size="1">
						...
						<?php if(is_array($opt) || $opt instanceof \think\Collection || $opt instanceof \think\Paginator): if( count($opt)==0 ) : echo "" ;else: foreach($opt as $key=>$v): if($v['opt'] == $row['answer_k']): ?>
								<option selected="selected" value="<?php echo $v['opt']; ?>"><?php echo $v['opt']; ?></option>
							<?php else: ?>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<!-- 加载编辑器的容器 -->
				<script id="container" name="answer_v" style="width:100%;height:400px;" type="text/plain"><?php echo $row['answer_v']; ?></script>
            </div>
        </div>
...
$("#form-article-add").submit(function(){
    ...
    $.ajax({
        type:"post",
        url: "<?php echo Url('Exambank/edit',array('id'=>input('id'))); ?>",
        data:data,
        dataType:'json',
        async:true,
