...
class User1 extends Base{
    ...
    public function wxlogin(){
        ...
        $results=db('user')->where(array('openid'=>$openid))->find();
    ...
    public function getUserIndex(){
        ...
        $data = D('Home/User')->getUserRank($time = null,$type = 1,$uid);

        //未读消息数量
        $msg_num = count(db('UserMsg')->field('id')->where('uid='.$uid.' AND is_see = 0')->select());
    ...
    public function getUserMsg(){
        ...
        $data = D('Home/UserMsg')->getUserMsg($uid);
