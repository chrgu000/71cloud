...
file_put_contents("b.txt",session('openid'));
    ...
    public function advice(Request $request)
    {
        ...
        $arr = ['uid' => $uid, 'content' => $content, 'ctime' => time()];
        if (!empty($link)){
            $arr['link'] = $link;
        }
        db('Advice')->data($arr)->insert();
     ...
     *
     */
    public function getPersonRank(Request $request)
    {
        $par  = $request->get();
        $uid  = $par['uid'];
        $time = empty($par['time']) ? '' : $par['time'];
        $type = empty($par['type']) ? 1 : $par['type'];

        if (empty($uid)) {
            return json(['code' => 0, 'msg' => '参数错误', 'data' => []]);
        }

        $data = model('home/User')->getUserRank($time, $type, $uid);
     ...
     *
     */
    public function getCompanyRank(Request $request)
    {
        $get  = $request->get();
        $uid  = $get['uid'];
        ...
        $data = model('home/User')->getCompanyRank($time, $type, $uid);
     ...
     *
     */
    public function getMyScoreDetail(Request $request)
    {

        $pre      = $request->get();
        $uid      = $pre['uid'];
        $p        = empty($pre['p']) ? 1 : $pre['p'];
        $pageSize = empty($pre['pageSize']) ? 6 : $pre['pageSize'];
        $time     = empty($pre['time']) ? null : $pre['time'];
        if (empty($uid)) {
            return json(['code' => 0, 'msg' => '参数错误', 'data' => '']);
        }

        $data = model('home/UserScore')->getScoreDetail($uid, $p, $pageSize, $time);
    ...
    public function editUserInfo(Request $request)
    {
        $pre      = $request->post();
        $uid      = $pre['uid'];
        $nickname = empty($pre['nickname']) ? '' : $pre['nickname'];
        $company  = empty($pre['company']) ? '' : $pre['company'];
        $face     = empty($pre['face']) ? '' : $pre['face'];
        $type     = $pre['type'];
        ...
        $user_model->where('id=' . $uid)->setField($field, $value);
    ...
    public function delMsg(Request $request)
    {
        ...
        $map['id']  = ['IN', $ids];
        $map['uid'] = $uid;
        $res        = Db::name('user_msg')->where($map)->delete();
        if ($res) {
