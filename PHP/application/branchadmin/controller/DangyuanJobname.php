...
class DangyuanJobname extends Base
{
    ...
    public function save(Request $request)
    {
        ...
        if (!empty($job_branch)) {
            ...
            // 查询不到直接新增一条记录即可
            model('DangyuanJobidBranchsid')->save(['branchs_id' => $branchs_id, 'job_ids' => $jobid]);
            $this->success('党员行政职称添加成功', 'lst');
        }
    }
    ...
    public function lst()
    {
        ...
        // 根据branchs_id获取支部名称
        $branchs_name = model('branchs')->where(['id' => $branchs_id])->value('branchs_name');
//        dump(model('branchs')->getLastSql());
//        dump($branchs_name);
//        die();
        // 获取当前支部所拥有的行政职称名称
