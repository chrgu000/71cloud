...
- (void)AddSegumentArray:(NSArray *)SegumentArray{
    ...
    for (int i = 0; i < SegumentArray.count; i++) {
        ...
        if (i == 0) {
            buttonDownView =[[UIView alloc]initWithFrame:CGRectMake(i *self.bounds.size.width/3 +(self.bounds.size.width/3 -60)/2 , self.bounds.size.height - 1, 60, 1)];
...
-(void)selectTheSegument:(NSInteger)segument{
    if (selectSeugment != segument){
        ...
        [UIView animateWithDuration:0.2 animations:^{
            [buttonDownView setFrame:CGRectMake(segument *self.bounds.size.width/3 + (self.bounds.size.width/3 - 60)/2, self.bounds.size.height - 1, 60, 1)];
        }];
    }
