...
//  TheParty
//
//  Created by macmini on 2018/8/8.
...
- (void)GreateUI{
    ...
    UIView *OldPassView = [[UIView alloc] initWithFrame:CGRectMake(38, CGRectGetMaxY(PassView.frame) +14, SCREEN_W -76, 40)];
    OldPassView.backgroundColor = COLORA(255, 255, 255, 0.7);
    OldPassView.layer.cornerRadius = 8;
    [mainScrollView addSubview:OldPassView];
    
    OldPass = [[UITextField alloc] initWithFrame:CGRectMake(5, 0,SCREEN_W -86, 40)];
    OldPass.delegate = self;
    OldPass.placeholder = @"请输入旧密码";
    OldPass.textColor = UIColorFromRGBA(0x333333, 1);
    OldPass.font = Font(15);
    [OldPass setValue:UIColorFromRGBA(0x999999, 1) forKeyPath:@"_placeholderLabel.textColor"];
    OldPass.secureTextEntry = YES;
    [OldPassView addSubview:OldPass];
    
    
    UIView *PassWordOneView = [[UIView alloc] initWithFrame:CGRectMake(38, CGRectGetMaxY(OldPassView.frame) +14, SCREEN_W -76, 40)];
...
- (void)Surelogin{
    if (CodeText.text.length ==0) {
        ...
        return;
    }else if (OldPass.text.length ==0){
        [self showError:@"请输入你的旧密码"];
        return;
    ...
    [self showHUD:nil];
    WEAKSELF
    NSDictionary *biz = @{@"tel":PhoneText.text,@"new_pass":passOne.text,@"code":CodeText.text,@"type":@"1",@"old_pass":OldPass.text};
    [ConnectionRequestMgr POSTSessionWithUrl:SecondForgetPass parameter:biz successBlock:^(NSDictionary *dict) {
        [weakSelf hideHUD];
        if ([dict[@"code"] integerValue] == 1) {
            [self showSuccess:@"修改密码成功！"];
            [self.navigationController popViewControllerAnimated:NO];
        }else{
            [weakSelf showError:dict[@"msg"]];
        }
