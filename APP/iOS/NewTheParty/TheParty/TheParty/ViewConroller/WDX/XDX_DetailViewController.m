...
- (void)creatDataTime{
    if (self.player.currentTime != 0) {
        ...
        NSString *timeCC = [NSString stringWithFormat:@"%zd",(int)self.player.currentTime];
        NSLog(@"%@",biz);
        [ConnectionRequestMgr POSTSessionWithUrl:SchoolSaveTime parameter:biz successBlock:^(NSDictionary *dict) {
            if ([dict[@"code"] integerValue] == 1) {
                for (int i = 0; i<self.DataArr.count; i++) {
                    XDX_DetailModel *model = self.DataArr[i];
                    if (i == isNum) {
                        model.see_time = timeCC;
                        NSLog(@"%@",model.see_time);
...
- (void)CreateList{
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(lineDown.frame), SCREEN_W, SCREEN_H) style:UITableViewStyleGrouped];
    ...
    self.mainScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0,CGRectGetMaxY(lineDown.frame), SCREEN_W, SCREEN_H)];
