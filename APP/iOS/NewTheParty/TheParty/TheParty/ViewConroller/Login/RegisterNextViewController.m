...
- (void)GreateUI{
    ...
    UIView *firstV = [[UIView alloc] initWithFrame:CGRectMake(35, 124, SCREEN_W - 70, 442)];
    ...
    UIView *SecondV = [[UIView alloc] initWithFrame:CGRectMake(25, 124, SCREEN_W - 50, 422)];
    ...
    NSArray *arr = @[@"请输入您的姓名",@"请输入密码",@"请再次确认密码",@"请选择组织",@"请选择政治面貌"];
    for (int i = 0; i < 5; i++) {
        
        UIView *BackView = [[UIView alloc] initWithFrame:CGRectMake(30, CGRectGetMaxY(logoL.frame)+30 + i*50, SCREEN_W -110, 35)];
        BackView.tag = 300 + i;
    ...
    sureB.frame = CGRectMake(30, 360, SCREEN_W -110, 35);
...
- (void)Surelogin{
    ...
    for (NSDictionary *dic in organizationTitleArr) {
        if ([organizationText.text isEqualToString:dic[@"company_name"]]) {
    ...
    [self showHUD:nil];
    WEAKSELF
    NSDictionary *biz = @{@"tel":self.phoneStr,@"password":passOne.text,@"password1":passTwo.text,@"username":NameText.text,@"branchs_id":organizationID,@"party_status":faceID};
    [ConnectionRequestMgr POSTSessionWithUrl:Nextregister parameter:biz successBlock:^(NSDictionary *dict) {
        [weakSelf hideHUD];
        if ([dict[@"code"] integerValue] == 1) {
